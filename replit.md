...
- **Onboarding & Free Trial:** Public `/onboarding` route with 3-step form (salon info → owner details → confirmation). Creates location + admin user + subscription in a DB transaction. 7-day free trial with `trialEndsAt` field on subscriptions table. Trial banner in DashboardLayout shows days remaining (green/yellow/red based on urgency). No credit card required. Session created via httpOnly cookie (no token exposure).
