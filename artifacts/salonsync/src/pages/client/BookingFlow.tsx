...
import { CheckCircle2, ChevronRight, User as UserIcon, Calendar as CalendarIcon, Scissors, CreditCard, Lock, AlertCircle } from "lucide-react";
...
function PaymentForm({
  ...
  const handlePay = useCallback(async () => {
    ...
    try {
      if (isMockMode) {
        // No real Stripe key — simulate success for dev/demo
    ...
    >
      ...
      <div className="bg-white/[0.03] border border-white/8 rounded-2xl p-5 mb-5">
        <div className="flex justify-between items-center text-sm mb-1">
          <span className="text-white/60">{appointment.services?.map(s => s.serviceName).join(", ")}</span>
      ...
      {isMockMode && (
        <div className="flex items-center gap-2.5 bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 mb-5">
          ...
          <p className="text-xs text-amber-300">
            <strong>Demo mode</strong> — no Stripe key configured. Payment will be simulated.
          </p>
        </div>
      )}
      ...
      {isMockMode && (
        <div className="border border-dashed border-white/15 rounded-xl px-4 py-4 mb-4 bg-white/[0.02] flex items-center gap-3 text-white/40">
          ...
          <span className="text-sm">4242 4242 4242 4242 (simulated)</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-1.5 text-xs text-white/30 mb-6">
        ...
        Payments are encrypted and secure {!isMockMode && "via Stripe"}
      </div>
...
export function BookingFlow() {
  ...
  const handlePaymentSuccess = () => {
    setSucceeded(true);
    setStep(6);
    toast.success("Payment confirmed!", { description: "Your appointment is booked." });
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between relative mb-12">
          ...
          <div
            ...
            style={{ width: `${((visibleStep - 1) / 4) * 100}%` }}
          />
          {steps.map(s => (
            <div key={s.num} className="flex flex-col items-center gap-2">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
                visibleStep >= s.num
                  ? "bg-primary text-white shadow-[0_0_15px_rgba(201,149,106,0.4)]"
                  : "bg-[#131D33] text-white/30 border border-white/8"
              }`}>
            ...
            <AnimatePresence mode="wait">
                    ...
                    {selectedService?.category === "HIGH_VALUE" && (
                      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 mb-6">
                        ...
                        <p className="text-sm text-amber-300/80">
                          This premium service requires a {selectedService.depositPercent}% deposit to secure your booking.
                        </p>
                      </div>
                    )}
              ...
              {step === 6 && (
                ...
                >
                  ...
                  <p className="text-white/50 mb-2">Your appointment has been confirmed and payment received.</p>
